...
public class DateTempAndFeelsLike {
    public static Map getWeatherMap(int cityID){
    ...
    public static List getWeatherList(int cityID){
        JSONObject data_obj = HttpGetFunction.doHttpGet(cityID); //entire json

        //initialising lists to store dates and temp
        List<String> dates_list = new ArrayList<>();
        List<String> temp_list = new ArrayList<>();
        List<String> feels_like_temp_list = new ArrayList<>();
        ...
        for (int i = 0; i < arr.size(); i++) {  //iterates through JSON array
            ...
            feels_like_temp_list.add(obj_main.get("feels_like").toString());

        }

        List<List<String>> listOfLists = new ArrayList<List<String>>();
        listOfLists.add(dates_list);
        listOfLists.add(temp_list);
        listOfLists.add(feels_like_temp_list);
